...
PokéAPI that is using in this project can be access [here](https://pokeapi.co/)  
...
```{r,echo = FALSE}
picture1 <- load.image('/Users/CKA/Desktop/Master 2021 Fall/ST558 data science for statistician/sp.jpg')
plot(picture1,axes=0)
```
```{r, echo = FALSE}
picture2 <- load.image('/Users/CKA/Desktop/Master 2021 Fall/ST558 data science for statistician/pika.jpg')
plot(picture2,axes=0)
```
...
[__knitr__](https://www.rdocumentation.org/packages/knitr/versions/1.30) Integrate R code into text documents  
[__httr__](https://httr.r-lib.org/) map closely to the underlying http protocol  
[__jsonlite__](https://cran.r-project.org/web/packages/jsonlite/vignettes/json-aaquickstart.html) convert between R objects and JSON without loss of type or information  
[__tidyverse__](https://www.tidyverse.org/) exploratory data analysis packages  
...
[__imager__](https://www.rdocumentation.org/packages/imager/versions/0.41.2) covert image to image data to put in documents  
...
__This is the function that inputs the endpoints, as well as the choices you can specify with those endpoints__  
*note:please read in all other functions listed in the __functions__ section before using this universal function* 
*note:all choices you can look up generated from output __tt__ under sections which is specified and described in each function*  
...
**2.For the berry data, if the choice is 0, it returns all the berries and their attributes.**  
...
**6.The total number of categories for the growth rate is 6. if you enter the number of choice greater than 6, it will return error, same rule applies to the other endpoint**  
...
**8.Recommendation: for the item and move endpoint, the time to get all the moves or items may take longer time. You can specify the choice to be __100__ or __200__** 
...
This Berry Data is kind of easy to visualize, there are not so many observations in the data set.  
The berry data is here because of its simplicity, I did look at other datasets such as pokemon species/items/moves that I get from the universal function. Those are more complicated and undtructured data sets.  
Anyway, doing this is a lot of fun. And there are  some places definitely need to be polished.  
For example, I need to find another way to speed up the time to get full item/move/pokemon species data...  
